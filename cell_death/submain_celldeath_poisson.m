function results = submain_celldeath_poisson(data,FACs_data_18hpi_vector,alpha_init,cell_line)
...
x0_timeindependent_inputindependent = log(alpha_init(1)); % 'a': alpha(t,i) = a
...
var_log_alpha_m1 = get_var_poisson(log_alpha_timeindependent_inputindependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
...
disp(['a = ',num2str(alpha_m1,'%2.4f'), ', CI[', num2str(CI_m1(1,1),'%2.4f'),', ',num2str(CI_m1(1,2),'%2.4f'), '] ']);
...
x0_timedependent_inputindependent = log(alpha_init(2:3)); % 'k','b': alpha(t,i) = b*k*t^(k-1)
...
[log_alpha_timedependent_inputindependent, LSE_percent_celldeath_timedependent_inputindependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputindependent);

AIC_model2 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputindependent)+2*k_timedependent_inputindependent;

var_log_alpha_m2 = get_var_poisson(log_alpha_timedependent_inputindependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
...
x0_timeindependent_inputdependent = log(alpha_init(4:5)); % log(c): alpha(t,i) = c*i^eps
...
[log_alpha_timeindependent_inputdependent, LSE_percent_celldeath_timeindependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timeindependent_inputdependent);

AIC_model3 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timeindependent_inputdependent)+2*k_timeindependent_inputdependent;

var_log_alpha_m3 = get_var_poisson(log_alpha_timeindependent_inputdependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
...
% initialize minimization - per capita death rate
x0_timedependent_inputdependent = log(alpha_init(6:8)); % 'k','d','eps': alpha(t,i) = b4*k4*t^(k4-1)*i^eps

k_timedependent_inputdependent = length(x0_timedependent_inputdependent);

% minimize sum squared error to get estimates of log(k), log(d), log(eps)
[log_alpha_timedependent_inputdependent, LSE_percent_celldeath_timedependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputdependent);

AIC_model4 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputdependent)+2*k_timedependent_inputdependent;

var_log_alpha_m4 = get_var_poisson(log_alpha_timedependent_inputdependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num);
...
for k = 1:length(data.hpi)
    ...
    m1_percentremaining = get_percentalive(alpha_m1,data.hpi(k),moi,1,1,which_distribution);
    m2_percentremaining = get_percentalive(alpha_m2,data.hpi(k),moi,1,2,which_distribution);
    m3_percentremaining = get_percentalive(alpha_m3,data.hpi(k),moi,1,3,which_distribution);
    m4_percentremaining = get_percentalive(alpha_m4,data.hpi(k),moi,1,4,which_distribution);
