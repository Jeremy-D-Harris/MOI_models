function results = submain_celldeath_negbinomial(data,FACs_data_18hpi_vector,alpha_init,r_dispersion_init,cell_line)
...
moi = linspace(0,15,100); % finer moi array for plotting models
...
percent_celldeath_allhpi = [data_3hpi; data_6hpi; data_12hpi; data_18hpi];
percent_cellsremaining_data_allhpi = 100-percent_celldeath_allhpi;
size_data_allhpi = size(percent_celldeath_allhpi);
...
x0_timeindependent_inputindependent = log([alpha_init(1),r_dispersion_m1]); % 'a': alpha(t,i) = a
...
[log_alpha_dispersion_timeindependent_inputindependent, LSE_percent_celldeath_timeindependent_inputindependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timeindependent_inputindependent);
...
var_log_alpha_m1 = get_var_negbinomial(log_alpha_dispersion_timeindependent_inputindependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
...
disp(['r (dispersion) = ',num2str(dispersion_m1,'%2.4f'), ', CI[', num2str(CI_m1(2,1),'%2.4f'),', ',num2str(CI_m1(2,2),'%2.4f'), '] ']);
...
k_dispersion_m2 = r_dispersion_init(2);

% initialize minimization - shape and scale parameters of the Weibull hazard function
x0_timedependent_inputindependent = log([alpha_init(2:3),k_dispersion_m2]); % 'k','b': alpha(t,i) = b*k*t^(k-1)
...
[log_alpha_dispersion_timedependent_inputindependent, LSE_percent_celldeath_timedependent_inputindependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputindependent);

AIC_model2 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputindependent)+2*k_timedependent_inputindependent;

var_log_alpha_m2 = get_var_negbinomial(log_alpha_dispersion_timedependent_inputindependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
...
disp(['r (dispersion) = ',num2str(dispersion_m2,'%2.4f'), ', CI[', num2str(CI_m2(3,1),'%2.4f'),', ',num2str(CI_m2(3,2),'%2.4f'), '] ']);
...
k_dispersion_m3 = r_dispersion_init(3);

% initialize minimization - per capita death rate
x0_timeindependent_inputdependent = log([alpha_init(4:5),k_dispersion_m3]); % 'c': alpha(t,i) = c*i^eps
...
[log_alpha_dispersion_timeindependent_inputdependent, LSE_percent_celldeath_timeindependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timeindependent_inputdependent);

AIC_model3 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timeindependent_inputdependent)+2*k_timeindependent_inputdependent;

var_log_alpha_m3 = get_var_negbinomial(log_alpha_dispersion_timeindependent_inputdependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
...
disp(['r (dispersion) = ',num2str(dispersion_m3,'%2.4f'), ', CI[', num2str(CI_m3(3,1),'%2.4f'),', ',num2str(CI_m3(3,2),'%2.4f'), '] ']);
...
k_dispersion_m4 = r_dispersion_init(4);

% minimize sum squared error to get estimates of 'k' and 'd'
% initialize minimization - per capita death rate
x0_timedependent_inputdependent = log([alpha_init(6:8),k_dispersion_m4]); % 'k','d': alpha(t,i) = b4*k4*t^(k4-1)*i^eps
...
[log_alpha_dispersion_timedependent_inputdependent, LSE_percent_celldeath_timedependent_inputdependent] = fminsearch(@(x)getfit_deathrate_dispersion(x,moi_vector_3hpi,moi_vector,percent_cellsremaining_data_allhpi,fitto_FACs_data_18hpi_vector,data.hpi,model_num,which_distribution),x0_timedependent_inputdependent);

AIC_model4 = n_pts_allhpi_FACs*log(LSE_percent_celldeath_timedependent_inputdependent)+2*k_timedependent_inputdependent;

var_log_alpha_m4 = get_var_negbinomial(log_alpha_dispersion_timedependent_inputdependent, n_pts_allhpi_FACs, moi_vector_3hpi, moi_vector, data.hpi, percent_cellsremaining_data_allhpi, fitto_FACs_data_18hpi_vector, model_num, cell_line);
...
disp(['r (dispersion) = ',num2str(dispersion_m4,'%2.4f'), ', CI[', num2str(CI_m4(4,1),'%2.4f'),', ',num2str(CI_m4(4,2),'%2.4f'), '] ']);
...
for k = 1:length(data.hpi)
    ...
    if k == 1
        ...
        subplot(1,4,k); plot(moi_vector_3hpi,percent_cellsremaining_data_allhpi(k,:),'.','Color',rgb_values(k,:),'MarkerSize',30); hold on;
    else
        % plot percent cells remaining data at all hours post infection
        subplot(1,4,k); plot(moi_vector,percent_cellsremaining_data_allhpi(k,:),'.','Color',rgb_values(k,:),'MarkerSize',30); hold on;
    end
end
...
for k = 1:length(data.hpi)
    ...
    m1_percentremaining = get_percentalive(alpha_m1,data.hpi(k),moi,dispersion_m1,1,2); % last two: which model, which distribution
    m2_percentremaining = get_percentalive(alpha_m2,data.hpi(k),moi,dispersion_m2,2,2);
    m3_percentremaining = get_percentalive(alpha_m3,data.hpi(k),moi,dispersion_m3,3,2);
    m4_percentremaining = get_percentalive(alpha_m4,data.hpi(k),moi,dispersion_m4,4,2);
    
    
    % fill in percent remaining (increasing MOI going down row) for each hpi (columns)
    m1_allhpi(:,k) = m1_percentremaining';
    m2_allhpi(:,k) = m2_percentremaining';
    m3_allhpi(:,k) = m3_percentremaining';
    m4_allhpi(:,k) = m4_percentremaining';
    
    figure(3); subplot(1,4,k);
    if k ==1
        
        r(1) = plot(moi,m1_percentremaining,'--','Color',rgb_values(k,:),'LineWidth',1); hold on;
        r(2) = plot(moi,m2_percentremaining,'Color',rgb_values(k,:),'LineWidth',1); hold on;
        r(3) = plot(moi,m3_percentremaining,'--','Color',rgb_values(k,:),'LineWidth',3); hold on;
        r(4) = plot(moi,m4_percentremaining,'Color',rgb_values(k,:),'LineWidth',3); hold on;
        
    else
        
        plot(moi,m1_percentremaining,'--','Color',rgb_values(k,:),'LineWidth',1); hold on;
        plot(moi,m2_percentremaining,'Color',rgb_values(k,:),'LineWidth',1); hold on;
        plot(moi,m3_percentremaining,'--','Color',rgb_values(k,:),'LineWidth',3); hold on;
        plot(moi,m4_percentremaining,'Color',rgb_values(k,:),'LineWidth',3); hold on;
    end
    title([num2str(this_hpi,'%2.0f'),' hpi']);
    axis([0 10 0 100]);
    xlabel('Bulk MOI'); ylabel('Percent cells remaining');
    ax = gca;
    ax.FontSize = 10;
    ax.FontWeight = 'bold';
    ax.FontName = 'Arial';
    ...
    if k == 4 % 18 hpi time point
        ...
        m1_percentremaining_infected_nb = get_percentaliveinfected(alpha_m1,data.hpi(k),moi,r_dispersion_m1,1,which_distribution); % which model, which distirbution
        m2_percentremaining_infected_nb = get_percentaliveinfected(alpha_m2,data.hpi(k),moi,k_dispersion_m2,2,which_distribution);
        m3_percentremaining_infected_nb = get_percentaliveinfected(alpha_m3,data.hpi(k),moi,k_dispersion_m3,3,which_distribution);
        m4_percentremaining_infected_nb = get_percentaliveinfected(alpha_m4,data.hpi(k),moi,k_dispersion_m4,4,which_distribution);
...
results.percentcells_alive_infected = [m1_percentremaining_infected_nb',m2_percentremaining_infected_nb',m3_percentremaining_infected_nb',m4_percentremaining_infected_nb'];
