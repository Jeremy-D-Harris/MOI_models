function sample_param_var_estimate = get_var_negbinomial(log_alpha, n_pts_allphi_FACs, moi_vector_3hpi, moi_vector, all_hpi, data_allhpi, FACs_data_18hpi, model_num, cell_line)
...
these_moi_vals_FACs = moi_vector(1,1:(end-3));
    ...
    if n ==4
        ...
        for k = 1:length(these_moi_vals)
            ...
            for m = 1:num_pars
                ...
                Jf_logtheta(counter,m) = (alpha(m)/h_small)*(get_percentalive(alpha_plus_h(1:(end-1)), this_hpi, this_moi, alpha_plus_h(end), model_num, which_distribution) - get_percentalive(alpha, this_hpi, this_moi, alpha(end), model_num, which_distribution));
                
            end
            
            
            var_estimate(1,counter_var) = (this_datum - get_percentalive(alpha(1:(end-1)), this_hpi, this_moi, alpha(end), model_num, which_distribution))^2;
        ...
        for k = 1:length(these_moi_vals_FACs)
            ...
            for m = 1:num_pars
                ...
                % partial derivative with chain rule to account for the transformation log(alpha)
                Jf_logtheta(counter,m) = (alpha(m)/h_small)*(get_percentaliveinfected(alpha_plus_h(1:(end-1)), this_hpi, this_moi, alpha_plus_h(end), model_num, which_distribution) - get_percentaliveinfected(alpha, this_hpi, this_moi, alpha(end), model_num, which_distribution));
                
            end
            
            
            var_estimate(1,counter_var) = (this_datum - get_percentaliveinfected(alpha(1:(end-1)), this_hpi, this_moi, alpha(end), model_num, which_distribution))^2;
            counter_var = counter_var+1;
