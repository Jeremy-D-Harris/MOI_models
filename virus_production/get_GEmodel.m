...
% cell death rate:
% 1:MDCK (time-dependent, input-independent)
% 2:A549 (time-independent, input-independent)
switch cell_line
    
    case 1 % MDCK
        
        % cell death rate: time-dependent, input-independent - a
        k = alpha(1); % shape parameter
        b = alpha(2); % scale parameters
        int_psi = b*time.^k; % integrating Weibull hazard function
        
    case 2 % A549
        
        % cell death rate: time-independent, input-independent - b*k*hpi^(k-1)
        a = alpha(1); % shape parameter
        int_psi = a*time; % integrating Weibull hazard function
